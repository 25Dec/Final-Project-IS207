...
module.exports = {
    ...
    authenticate: function (loginName, password, callback) {
        ...
        try {
            ...
            User.findOne( {loginName: loginName}, function (error, user) { // , status: Constant.STATUS_ENUM[0], isEmailVerify: true
                if (error) {
                    return callback(8, 'find_fail', 420, error, null);
                }
    ...
    checkUserValidAvailable: function (accessUserId, accessUserRight, accessLoginName, callback) {
        try {
            ...
            User.findOne({loginName: accessLoginName, _id: accessUserId, status: Constant.STATUS_ENUM[0],userRight: accessUserRight }, function (error, user) {
                if (error) {
                    return callback(8, 'find_fail', 420, error, null);
                }
                 //console.log(user);
